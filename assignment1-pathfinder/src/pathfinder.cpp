        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            ...
            if (auto opt = get_nearby_node(GetMousePosition()))
            {
                ...
                if (next == end)
                {
                    ...
                    //high score mechanic, only changes the high score if its greater than the current one
                    if (score > high_score)
                    {
                        high_score = score;
                    ...
                    // randomizes the start and end node once the end node is reached
                    start = nodes[rand() % nodes.size()]; 
                    end = nodes[rand() % nodes.size()];
                    
                    //stops the start and end from being set to the same node
                    if (start == end) 
                    {  //start and end are not the same node
                        end = nodes[rand() % nodes.size()];
                    }
